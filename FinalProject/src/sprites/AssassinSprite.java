...
public class AssassinSprite extends Sprite {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -6288123448853605193L;
	...
	public AssassinSprite() {
		if (sheet == null) // load the sprite sheet into memory
			try {
				sheet = ImageIO.read(new File("images/AssassinWalkingAlpha.png"));
			} catch (Exception e) {
			}
		;
	...
	public Image getImage() {
		...
		BufferedImage temp = sheet.getSubimage(leftBorder + col * width, row
				* (height + vertBorder), width - offset, height);

		// Up-scaling the frame by 2x
		return temp.getScaledInstance(2 * width, 2 * height,
				Image.SCALE_DEFAULT);
	}
